  ...
  def createTopics(timeout: Int,
    ...
    val metadata = toCreate.values.map(topic =>
      if (metadataCache.contains(topic.name)) {
        val e = new TopicExistsException(s"Topic '${topic.name}' already exists.")
        ...
        case e: TopicExistsException =>
          // We checked whether the topic exists in the controller's metadata cache above, so receiving an error that the
          // topic exists is unexpected. However, it may be the case that the topic is currently undergoing deletion and
          // has been removed from the cache but not fully garbage collected, in which case we'll instruct the client to retry.
          info(s"Error processing create topic request $topic", e)
          CreatePartitionsMetadata(topic.name, Map(), ApiError.fromThrowable(new UnknownTopicOrPartitionException(
            "Topic couldn't be created, may already exist or is being garbage collected", e.getCause)))
        case e: ApiException =>
          info(s"Error processing create topic request $topic", e)
          CreatePartitionsMetadata(topic.name, Map(), ApiError.fromThrowable(e))
