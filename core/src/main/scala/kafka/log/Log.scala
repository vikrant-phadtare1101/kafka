...
class Log(@volatile var dir: File,
    ...
    maybeHandleIOException(s"Exception while reading from $topicPartition in dir ${dir.getParent}") {
      ...
      while (segmentEntry != null) {
        ...
        if (fetchInfo == null) {
          ...
          if (isolation != FetchLogEnd) {
            recordConsumerFetchLagMetrics(segmentEntry.getKey, segment.size, endOffset, fetchInfo)
          }
  ...
  private def recordConsumerFetchLagMetrics(segmentBaseOffset: Long, segmentSizeInBytes: Long, 
                                            endOffset: Long, fetchInfo: FetchDataInfo): Unit = {
    // record fetch time lag in milliseconds 
    // time lag = activeSegment.maxTimestampSoFar - MaximumTimestamp of first batch fetched for the startOffset  
    val iterator = fetchInfo.records.batches.iterator
    if (iterator.hasNext()) {
      val batch = iterator.next()
      if (batch.magic >= RecordBatch.MAGIC_VALUE_V2) {
        val startOffsetTimestamp = batch.maxTimestamp()
        val maxTimestampSoFar = activeSegment.maxTimestampSoFar
        if (startOffsetTimestamp != -1 && maxTimestampSoFar != -1) {
          val fetchLagInMS = maxTimestampSoFar - startOffsetTimestamp
          brokerTopicStats.allTopicsStats.consumerFetchLagTimeInMs.update(fetchLagInMS)
        }
      }
    } 
    
    // record fetch lag in bytes
    // consumerFetchLagInBytes = Sum of the newer segments (if any) + 
    //                          (segment_size - relative position of fetched offset in segment)
    val consumerFetchLagInBytes = {
      val nextSegmentEntry = segments.higherEntry(segmentBaseOffset)
      val segmentLag = segmentSizeInBytes - fetchInfo.fetchOffsetMetadata.relativePositionInSegment.toLong
      if (nextSegmentEntry == null) {
         segmentLag
      } else {
         val newerSegmentsSize = logSegments(nextSegmentEntry.getKey, endOffset).map(_.size.toLong).sum
         segmentLag + newerSegmentsSize
      }
    }
    brokerTopicStats.allTopicsStats.consumerFetchLagInBytes.update(consumerFetchLagInBytes)
  }
