...
/*
*  The test validates the fetch lag (time and byte) metrics 
*  The log is created with multiple segments and the expected lag 
*  is validated.
*  Time Lag: The test messages are created with 1 ms different in event time which is then 
*  used to validate the time lag
*  Byte Lag: The record are of 1 message size and each segment of 2 records. This information 
*  is used to calculate the expected lag
*/
@Test
  def testConsumerFetchLagMetrics(): Unit = {
    ...
    val messageSize = createRecords.sizeInBytes
    val msgPerSeg = 2
    val segmentSize = msgPerSeg * (messageSize)
    val logConfig = LogTest.createLogConfig(segmentBytes = segmentSize)
    //  create a log
    val log = createLog(logDir, logConfig)
    val messageEventTimestampDelta = 1
    // append records to log
    for (numMessages <- 1 until 10) {
      log.appendAsLeader(createRecords, leaderEpoch = 0)
      mockTime.sleep(messageEventTimestampDelta)
    }

    def assertFetchLagMetrics(): Unit = {
      (log.highWatermark-1 to log.logStartOffset by -1).foreach { offset =>
        assertNonEmptyFetch(log, offset, FetchHighWatermark)
       
        val expectedByteLag = messageSize * ((log.highWatermark -1) - offset + 1)
        val expectedTimeLag = messageEventTimestampDelta*((log.highWatermark - 1) - offset)
        val lagBytes = brokerTopicStats.allTopicsStats.consumerFetchLagInBytes.getSnapshot().getValues().lastOption.getOrElse(-1)
        val lagTimeInMs = brokerTopicStats.allTopicsStats.consumerFetchLagTimeInMs.getSnapshot().getValues().lastOption.getOrElse(-1)

        assertEquals("byte lag last histogram value", expectedByteLag.toDouble, lagBytes)
        assertEquals("time lag last histogram value", expectedTimeLag.toDouble, lagTimeInMs)
      }
    }

    log.updateHighWatermark(9L)
    assertFetchLagMetrics()
  }
