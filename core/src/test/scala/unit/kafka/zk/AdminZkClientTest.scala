...
import org.apache.kafka.common.errors.{InvalidReplicaAssignmentException, InvalidTopicException, TopicExistsException, UnknownTopicOrPartitionException}
...
class AdminZkClientTest extends ZooKeeperTestHarness with Logging with RackAwareTest {
  ...
  def testTopicDeletion(): Unit = {
    ...
    // attempt to delete the topic again
    intercept[TopicAlreadyMarkedForDeletionException] {
      adminZkClient.deleteTopic(topic)
    }

    assertTrue(zkClient.topicExists(topic))
    assertTrue(zkClient.isTopicMarkedForDeletion(topic))
    assertEquals(zkClient.getTopicDeletions, Seq(topic))

    intercept[TopicExistsException] {
      adminZkClient.validateTopicCreate(topic, Map.empty, new Properties)
    }

    zkClient.createControllerEpochRaw(0)
    zkClient.deleteTopicZNode(topic, 0)
    zkClient.deleteTopicConfigs(Seq(topic), 0)

    // the topic's znode was deleted, however its deletion marker still exists
    intercept[UnknownTopicOrPartitionException] {
      adminZkClient.validateTopicCreate(topic, Map.empty, new Properties)
    }

    zkClient.deleteTopicDeletions(Seq(topic), 0)

    // verify topic can be created again
    adminZkClient.createTopic(topic, 3, 1)
  }
